...
public:
  ...
  int Size() { return num; }
  Status Get_Data(const int i, Data_Type &get) {
    if (i <= 0) {
      get = 0;
  ...
  Status Search_data(const Data_Type target_data, int &target_num) {
    int i = 0;
    bool is_found = false;
    for (; i < num; i++) {
    ...
    target_num = is_found ? i + 1 : 0;
    return is_found ? Status::Success : Status::Waring_Not_Found;
  }
  Status Insert(const Data_Type new_data, const int i) {
    if (i < 1) {
      return Status::Error_Invalid_Index;
...
Node_Test: {
  ...
  while (true) {
    ...
    case 3:
      std::cout << "输入下标：";
      std::cin >> target_num;
      if (node_test.Get_Data(target_num, target_data) ==
          MyLib::Status::Success) {
        std::cout << "获得的元素为：" << target_data << std::endl;
      } else {
    ...
    case 4:
      std::cout << "输入要查找的元素：";
      std::cin >> target_data;
      if (node_test.Search_data(target_data, target_num) ==
          MyLib::Status::Success) {
        std::cout << "该元素第一次出现在下标" << target_num << std::endl;
      } else {
    ...
    case 5:
      ...
      std::cout << "输入插入的下标：";
      std::cin >> target_num;
      if (node_test.Insert(target_data, target_num) == MyLib::Status::Success) {
    ...
    case 6:
      std::cout << "输入删除元素的下标：";
      std::cin >> target_num;
      if (node_test.Delete(target_num) == MyLib::Status::Success) {
...
SeqList_Test: {
  ...
  while (true) {
    ...
    case 3:
      std::cout << "输入下标：";
      std::cin >> target_num;
      if (seqlist_test.Get_Data(target_num, target_data) ==
          MyLib::Status::Success) {
        std::cout << "获得的元素为：" << target_data << std::endl;
      } else {
        std::cout << "获取失败" << std::endl;
    ...
    case 4:
      std::cout << "输入要查找的元素：";
      std::cin >> target_data;
      if (seqlist_test.Search_data(target_data, target_num) ==
          MyLib::Status::Success) {
        std::cout << "该元素第一次出现在下标" << target_num << std::endl;
      } else {
        std::cout << "查找失败" << std::endl;
    ...
    case 5:
      ...
      std::cout << "输入插入的下标：";
      std::cin >> target_num;
      if (seqlist_test.Insert(target_data, target_num) ==
          MyLib::Status::Success) {
        std::cout << "插入成功" << std::endl;
      } else {
        std::cout << "插入失败" << std::endl;
    ...
    case 6:
      std::cout << "输入删除元素的下标：";
      std::cin >> target_num;
      if (seqlist_test.Delete(target_num) == MyLib::Status::Success) {
        std::cout << "删除成功" << std::endl;
      } else {
        std::cout << "删除失败" << std::endl;
      }
