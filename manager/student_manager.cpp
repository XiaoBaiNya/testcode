...
private:
  ...
  /**
   * @brief
   * 删除所有链表，新建next到下一个新节点，再删除上一个节点，head跳转到next
   *
   * @param head 链表头
   */
...
public:
  ...
  /**
   * @brief 读取data.txt里的内容到链表里，排序输出链表，再删除链表
   */
  void scan_information() {
    stu *head = getdataFile();
    analyzeStudent(head);
    deletelist(head);
  }
  /**
   * @brief 输入学生信息到链表中，再把链表内容保存到data.txt里
   */
  void add() {
  ...
  /**
   * @brief 读取data.txt里的内容到链表里，选择一个学生并且判断是否删除
   */
  void delete_select() {
    stu *head = getdataFile();
  ...
  /**
   * @brief 读取data.txt里的内容到链表里，选择一个学生并且判断是否更改
   */
  void change_select() {
    stu *head = getdataFile();
...
int main() {
  ...
  while (true) {
    ...
    std::cin >> n;
    if (n == 1) {
      manager.scan_information();
    } else if (n == 2) {
      manager.add();
    } else if (n == 3) {
      manager.delete_select();
    } else if (n == 4) {
      manager.change_select();
    } else if (n == 5) {
      manager.deletedatafile();
    } else if (n == 0) {
      std::cout << std::endl
